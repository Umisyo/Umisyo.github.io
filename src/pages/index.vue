<template lang="pug">
  div.top
    img.top__icon(src="~/assets/images/icon.webp" width="10rem" height="10rem")
    h1.top__name Souta Kusunoki
    div.top__social
      a.top__social-link(href="http://github.com/Umisyo")
        img(src="~/assets/images/github-icon.webp" width="4em" height="4em")
      a.top__social-link-instagram(href="http://instagram.com/ihcamonoihS")
        img(src="~/assets/images/instagram-icon.webp" width="4em" height="4em")
      a.top__social-link(href="http://twitter.com/ihcamonoihS")
        img(src="~/assets/images/twitter-icon.webp" width="4em" height="4em")
      a.top__social-link(href="http://facebook.com/ihcamonoihS")
        img(src="~/assets/images/facebook-icon.webp" width="4em" height="4em")
</template>
